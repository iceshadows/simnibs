...
def tms_sphere(sphere3_msh):
    ...
    dipole_pos = np.array([0.0, 0.0, 300])
    ...
    E_analytical = analytical_solutions.tms_E_field(
        dipole_pos * 1e-3, dipole_moment, didt, pos * 1e-3
    )
...
class Testcalc_fields:
    def test_calc_vEJgs(self, sphere3_msh):
        ...
        assert np.allclose(
            m.field["magnE"].value, np.linalg.norm(E.value, axis=1) * 1e3
        )
        assert np.allclose(
            m.field["magnJ"].value,
    ...
    def test_calc_dadt(self, sphere3_msh):
        ...
        dadt = 0.2 * sphere3_msh.nodes.node_coord
        ...
        m = fem.calc_fields(potential, "vDJEgsej", cond, dadt=dadt, units="m")
        assert np.allclose(m.field["v"].value, potential.value)
...
class TestAssemble:
    ...
    def test_vol(self, sphere3_msh):
        ...
        assert np.isclose(np.sum(v), 4.0 / 3.0 * np.pi * (95**3), rtol=1e-2)

    def test_mass_matrix(self, sphere3_msh):
        v = np.ones(sphere3_msh.nodes.nr)
        M = fem.assemble_diagonal_mass_matrix(sphere3_msh)
        assert np.isclose(v.dot(M.dot(v)), 1e-9 * 4.0 / 3.0 * np.pi * 95**3, rtol=1e-2)
...
class TestFEMSystem:
    ...
    def test_set_up_tms(self, tms_sphere):
        ...
        m.elmdata = [
            mesh_io.ElementData(E_analytical, "analytical"),
            mesh_io.ElementData(E, "E_FEM"),
            mesh_io.ElementData(
                E_analytical + dAdt.node_data2elm_data().value, "grad_analytical"
            ),
            dAdt,
        ]
    ...
    def test_dirichlet_problem_cube(self, cube_lr):
        ...
        bcs = [
            fem.DirichletBC(top, np.ones(len(top))),
    ...
    def test_dirichlet_problem_sphere(self, sphere3_msh):
        ...
        v_analytical = analytical_solutions.potential_3layers_surface_electrodes(
            [85, 90, 95], [1.0, 0.01, 1.0], [0, 0, -95], [0, 0, 95], m.nodes.node_coord
        )
        v_analytical /= v_analytical[anode - 1]
        v_analytical -= v_analytical[0]
        x -= x[0]
        m.nodedata = [
            mesh_io.NodeData(v_analytical, "Analytical"),
            mesh_io.NodeData(x, "FEM"),
        ]
        # mesh_io.write_msh(m, '~/Tests/fem.msh')
        m = m.crop_mesh(3)
        assert rdm(m.nodedata[0].value, m.nodedata[1].value) < 0.1
    ...
    def test_solve_assemble_neumann_nodes(self, current_type, cube_msh):
        ...
        if current_type == "float":
            ...
            currents = [
                np.ones_like(nodes_top) / nodes_top.size,
                -np.ones_like(nodes_bottom) / nodes_bottom.size,
            ]
            weigh_by_area = False
        else:
            raise ValueError
        S = fem.TDCSFEMNeumann(
            m, cond, electrodes[0], input_type="nodes", weigh_by_area=weigh_by_area
        )
...
class TestTMS:
    ...
    def test_tms_coil(self, mock_set_up, tms_sphere):
        ...
        fem.tms_coil(m, cond, None, "coil.ccd", "EJ", [matsimnibs], [didt], [fn_out])
        E = mesh_io.read_msh(fn_out).field["E"].value
        os.remove(fn_out)
        mock_set_up.assert_called_once_with("coil.ccd", m, didt, matsimnibs)
        assert rdm(E, E_analytical) < 0.2
        assert np.abs(mag(E, E_analytical)) < np.log(1.1)
    ...
    def test_leadfield(self, input_type, n_workers, field, post_pro, cube_msh):
        ...
        if not post_pro:
            ...
            with h5py.File(fn_hdf5, "r") as f:
                ...
                assert (
                    rdm(f[dataset][0, ...], np.tile([0.0, 100, 0.0], (n_roi, 1))) < 0.2
                )
                assert mag(
                    f[dataset][0, ...], np.tile([0.0, 100, 0.0], (n_roi, 1))
                ) < np.log(1.1)
        if post_pro:
            with h5py.File(fn_hdf5, "r") as f:
                assert f[dataset].shape == (2, 10, 3)
                assert rdm(f[dataset][0, ...], np.tile([0.0, 200, 0.0], (10, 1))) < 0.2
                assert mag(
                    f[dataset][0, ...], np.tile([0.0, 200, 0.0], (10, 1))
                ) < np.log(1.1)
...
class TestTMSMany:
    ...
    def test_many_simulations(self, mock_set_up, n_workers, post_pro, tms_sphere):
        ...
        with h5py.File(fn_hdf5, "r") as f:
            for E in f[dataset]:
                if post_pro:
                    assert rdm(E, post(E_analytical[roi_select])) < 0.3
                    assert mag(E, post(E_analytical[roi_select])) < np.log(1.1)
                else:
                    assert rdm(E, E_analytical[roi_select]) < 0.3
...
class TestDipole:
    ...
    def test_single_dipole(
        self, source_model, dipole_direction, pos_target, sphere3_msh
    ):
