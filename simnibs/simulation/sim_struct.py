...
class SESSION(object):
    ...
    def _prepare(self):
        ...
        if os.path.isfile(self.fnamehead):
            ...
            raise IOError("Could not find head mesh file: {0}".format(self.fnamehead))
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True):
        ...
        if self.map_to_surf or self.map_to_fsavg or self.map_to_vol or self.map_to_MNI:
            if not self.subpath:
                ...
                raise IOError(
                    "Cannot run postprocessing: {0} is not a " "directory".format(
                        self.subpath
                    )
                )
    ...
    def read_mat_struct(self, mat):
        ...
        if (
            self.tissues_in_niftis is not None
            and type(self.tissues_in_niftis[0]) == str
            and self.tissues_in_niftis[0] == "a"
        ):
            self.tissues_in_niftis = "all"

        self.fields = try_to_read_matlab_field(mat, "fields", str, self.fields)
        ...
        if len(mat["poslist"]) > 0:
            for PL in mat["poslist"][0]:
                if PL["type"][0] == "TMSLIST":
                    ...
                    raise IOError("poslist type is not of type TMSLIST or TDCSLIST")
    ...
    def _set_logger(self, fname_prefix="simnibs_simulation", summary=True):
        ...
        log_fn = os.path.join(
            self.pathfem, fname_prefix + "_{0}.log".format(self.time_str)
        )
            ...
            if t == "Fiducial":
                if n in ["Nz", "Iz", "LPA", "RPA"]:
                    ...
                    logger.warning(
                        ...
                        "Acceptable fiducuals are: {1}".format(
                            n, ["Nz", "Iz", "LPA", "RPA"]
                        )
                    )
...
class SimuList(object):
    ...
    def postprocess(self, value):
        valid_postptocessing = set(["v", "E", "e", "J", "j", "g", "s", "D"])
        if not set(value).issubset(valid_postptocessing):
            raise ValueError(
                "postprocessing operation: {0} \n"
                "Not Supported, Supported operations are: "
                "{1}".format(
                    list(set(value) - valid_postptocessing), list(valid_postptocessing)
                )
            )
        else:
    ...
    def read_cond_mat_struct(self, mat_struct):
        ...
        self.name = try_to_read_matlab_field(mat_struct, "name", str, self.name)
    ...
    def cond2elmdata(self, mesh=None, excentricity_scale=None, logger_level=20):
        ...
        if self.anisotropy_type == "scalar":
            ...
            return simnibs.utils.cond_utils.cond2elmdata(
                mesh,
                cond_list,
            ...
            logger.log(
                level,
                "Using anisotropic volume normalized conductivities based on the file:"
                " {0}".format(self.fn_tensor_nifti),
            )
            image, affine = self._get_vol_info()
            return simnibs.utils.cond_utils.cond2elmdata(
            ...
            logger.log(
                level,
                "Using isotropic mean conductivities based on the file:" " {0}".format(
                    self.fn_tensor_nifti
                ),
            )
            image, affine = self._get_vol_info()
            return simnibs.utils.cond_utils.cond2elmdata(
    ...
    def _get_vol_info(self):
        ...
        if not self.fn_tensor_nifti:
            raise ValueError(
                "could not get anisotropy information: " "fn_tensor_nifti not set"
            )

        fn_nifti = os.path.abspath(os.path.expanduser(self.fn_tensor_nifti))
        if not os.path.isfile(fn_nifti):
            raise ValueError(
                "Could not find file '{0}' to get anisotropy " "information".format(
                    self.fn_tensor_nifti
                )
            )
    ...
    def _write_conductivity_to_hdf5(self, fn_hdf5, path="cond/"):
        ...
        with h5py.File(fn_hdf5, "a") as f:
            ...
            g.create_dataset(
                "names", data=np.array([c.name for c in self.cond], dtype=np.bytes_)
            )
            g.create_dataset(
                "distribution_types",
                data=np.array(
                    [c.distribution_type for c in self.cond], dtype=np.bytes_
                ),
            )

            distribution_parameters = np.nan * np.zeros(
                (len(self.cond), 4), dtype=float
            )
            for i, c in enumerate(self.cond):
                distribution_parameters[i, : len(c.distribution_parameters)] = (
                    c.distribution_parameters
                )
            g.create_dataset("distribution_parameters", data=distribution_parameters)
    ...
    def _get_conductivity_from_hdf5(self, fn_hdf5, path="cond/"):
        ...
        with h5py.File(fn_hdf5, "r") as f:
            try:
                ...
                raise IOError(
                    "Could not find the group {0} " "in the HDF5 file".format(path)
                )
            ...
            for i, c in enumerate(self.cond):
                ...
                self.cond[i].distribution_parameters = dist_p_array[i][
                    ~np.isnan(dist_p_array[i])
                ].tolist()
...
class TMSLIST(SimuList):
    ...
    def resolve_fnamecoil(self):
        ...
        if os.path.isfile(fnamecoil):
            ...
            if os.path.isfile(fnamecoil):
                ...
                raise IOError("Could not find coil file: {0}".format(self.fnamecoil))
    ...
    def run_simulation(self, fn_simu, cpus=1, view=True):
        ...
        fn_simu = [
            "{0}-{1:0=4d}_{2}_".format(fn_simu, i + 1, coil_name)
    ...
    def __str__(self):
        string = (
            ...
            " nr coil positions: {2} \n"
            " anisotropy: {3}"
            "".format(self.type, self.fnamecoil, len(self.pos), self.anisotropy_type)
        )
        return string
...
class POSITION(object):
    ...
    def matsimnibs_is_defined(self):
        if isinstance(self.matsimnibs, np.ndarray):
            if self.matsimnibs.ndim == 2 and self.matsimnibs.shape == (4, 4):
                return True
        elif (
            self.matsimnibs
            and np.array(self.matsimnibs).ndim == 2
            and np.array(self.matsimnibs).shape == (4, 4)
        ):
            return True
        else:
            return False
    ...
    def __eq__(self, other):
        if (
            ...
            or self.matsimnibs != other.matsimnibs
            or self.didt != other.didt
        ):
            return False
...
class TDCSLIST(SimuList):
    ...
    def _prepare(self):
        ...
        for i in self.unique_channels:
            ...
            if self.cond[(ElementTags.ELECTRODE_RUBBER_START - 1) + i].value is None:
                self.cond[(ElementTags.ELECTRODE_RUBBER_START - 1) + i].name = (
                    str(i) + " el"
                )
                self.cond[
                    (ElementTags.ELECTRODE_RUBBER_START - 1) + i
                ].value = self.cond[(ElementTags.ELECTRODE_RUBBER_START - 1)].value
            if self.cond[(ElementTags.SALINE_START - 1) + i].value is None:
                self.cond[(ElementTags.SALINE_START - 1) + i].name = (
                    str(i) + " gel_sponge"
                )
                self.cond[(ElementTags.SALINE_START - 1) + i].value = self.cond[
                    (ElementTags.SALINE_START - 1)
                ].value
        ...
        if len(self.unique_channels) != len(self.currents):
            raise ValueError(
                "Number of unique channels is not equal to the number of"
                "current values"
            )
    ...
    def run_simulation(self, fn_simu, cpus=1, view=True):
        ...
        v.add_view(ColormapNumber=10, ColormapAlpha=0.5, Visible=1)  # el_currents
        if add_scalp_to_geo:
            v.add_view(ColormapNumber=8, ColormapAlpha=0.3, Visible=0, ShowScale=0)
    ...
    def run_gpc(self, fn_simu, cpus=1, tissues=[ElementTags.GM], eps=1e-2):
        ...
        gPC_regression = run_tcs_gpc(self, fn_simu, cpus=cpus, tissues=tissues, eps=eps)
        return gPC_regression
    ...
    def _electrode_current_geo(self, m, fn_out):
        ...
        mesh_io.write_geo_triangles(
            triangles - 1, m.nodes.node_coord, fn_out, values, "electrode_currents"
        )
...
class ELECTRODE(object):
    ...
    def shape(self, s):
        if s in ["rectangle", "rect", "ellipse", "custom", ""]:
            ...
            raise ValueError(
                "Electrode shape must be" "'rect', 'ellipse', 'custom' or ''"
            )
    ...
    def read_mat_struct(self, el):
        ...
        self.dimensions = try_to_read_matlab_field(
            el, "dimensions", list, self.dimensions
        )
        ...
        self.dimensions_sponge = try_to_read_matlab_field(
            el, "dimensions_sponge", list, self.dimensions_sponge
        )
...
class LEADFIELD:
    ...
    def read_mat_struct(self, mat):
        ...
        self.interpolation_subsampling = try_to_read_matlab_field(
            mat, "interpolation_subsampling", int, self.interpolation_subsampling
        )
...
class TDCSLEADFIELD(LEADFIELD):
    ...
    def _add_electrodes_from_cap(self):
        ...
        type_, coordinates, extra, name, _, _ = read_csv_positions(self.eeg_cap)
        ...
        try:
            ...
            self.electrode = [copy.deepcopy(self.electrode) for i in range(count_csv)]
            count_struct = len(self.electrode)
    ...
    def _add_el_conductivities(self):
        ...
        for i in self.unique_channels:
            ...
            if self.cond[(ElementTags.SALINE_START - 1) + i].value is None:
                self.cond[(ElementTags.SALINE_START - 1) + i].name = "gel_sponge" + str(
                    i + 1
                )
                self.cond[(ElementTags.SALINE_START - 1) + i].value = self.cond[
                    (ElementTags.SALINE_START - 1)
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True):
        ...
        def contains_volume(x):
            return any([ElementTags.TH_START <= i <= ElementTags.TH_END for i in x])

        def contains_surface(x):
            return any(
                [
                    ElementTags.TH_SURFACE_END >= i >= ElementTags.TH_SURFACE_START
                    for i in x
                ]
            )
        ...
        if self.interpolation:
            assert (
                self.interpolation_tissue
            ), "Interpolation tissues must be specified when interpolating solution"
            assert not contains_surface(
                self.interpolation_tissue
            ), "Interpolation tissues must be volumes"
            if self.tissues:
                assert not contains_volume(
                    self.tissues
                ), "Mixing volume ROIs and surface interpolation is not allowed"
        else:
            ...
            assert contains_volume(self.tissues) ^ contains_surface(
                self.tissues
            ), "Tissue ROIs must be either surfaces OR volumes"
        ...
        if self.eeg_cap is not None:
            if os.path.isfile(self.eeg_cap):
                ...
                raise IOError("Could not find EEG cap file: {0}".format(self.eeg_cap))
        try:
            len(self.electrode)
        except TypeError:
            raise ValueError("Please define either an EEG cap or a list of electrodes")
        ...
        # Handle the ROI
        if np.any(np.array(self.tissues) >= ElementTags.TH_SURFACE_START) and np.any(
            np.array(self.tissues) <= ElementTags.TH_END
        ):
            raise ValueError("Mixing Volumes and Surfaces in ROI!")

        if self.field != "E" and self.field != "J":
            raise ValueError(
                "field parameter should be E or J. " "found: {0}".format(self.field)
            )
        ...
        if has_electrodes:
            ...
            scalp_electrodes = w_elec.crop_mesh(
                [ElementTags.SCALP_TH_SURFACE] + electrode_surfaces
            )
        ...
        with h5py.File(fn_hdf5, "a") as f:
            ...
            if self.interpolation is None:
                ...
                f[dset].attrs["interpolation"] = (
                    "middle gm" if self.interpolation == "middle gm" else "True"
                )
            f[dset].attrs["interpolation_subsampling"] = (
                self.interpolation_subsampling
                if self.interpolation_subsampling is not None
                else "None"
            )
    ...
    if isinstance(pos, str):
        if eeg_cap:
            ...
            try:
                ...
                raise ValueError(
                    "Could not find position {0} in cap {1}".format(pos, eeg_cap)
                )
        else:
            raise ValueError(
...
def _show_for_debugging(
    m, sph_centre, radius, P_centre, P_surround, surround_fit, M_sph
):
    ...
    mesh_io.write_geo_spheres(
        sph_centre.reshape((1, 3)), fn_geo, name="center", mode="bw"
    )
    mesh_io.write_geo_spheres(
        (M_sph @ [radius, 0, 0, 1])[:3].reshape((1, 3)), fn_geo, name="x", mode="ba"
    )
    mesh_io.write_geo_spheres(
        (M_sph @ [0, radius, 0, 1])[:3].reshape((1, 3)), fn_geo, name="y", mode="ba"
    )
    mesh_io.write_geo_spheres(
        (M_sph @ [0, 0, radius, 1])[:3].reshape((1, 3)), fn_geo, name="z", mode="ba"
    )

    TH_DEBUG = np.arange(-1.0, 1.01, 0.1) * np.pi
    PHI_DEBUG = np.arange(0.0, 1.01, 0.05) * 2 * np.pi
    ...
    mesh_io.write_geo_spheres((M_sph @ pts)[:3, :].T, fn_geo, name="sphere", mode="ba")

    mesh_io.write_geo_spheres(
        P_centre.reshape((1, 3)), fn_geo, name="centre", mode="ba"
    )
    for i in range(len(P_surround)):
        mesh_io.write_geo_spheres(
            P_surround[i].reshape((1, 3)), fn_geo, name="surr " + str(i), mode="ba"
        )
    ...
    for i in range(len(surround_fit)):
        ...
        tmp_arc = np.vstack(
            (
                ...
                * np.sin(
                    tmp_theta_z0
                    + (tmp_theta - tmp_theta_z0) * np.arange(N_pts) / (N_pts - 1)
                )
                + tmp_centre[0],
                np.zeros((1, N_pts)),
                tmp_r
...
def get_surround_pos(
    ...
    pos_dir_1stsurround=None,
    phis_surround=None,
    tissue_idx=ElementTags.SCALP_TH_SURFACE,
    DEBUG=False,
):
    """
    ...
    for phi in range(N):
        ...
        arc = np.vstack(
            (
                r_sph * np.sin(theta_on_sph * np.arange(N_pts) / (N_pts - 1)),
                np.zeros((1, N_pts)),
        ...
        tmp = np.array(
            (
                r_arc * np.sin(theta_on_arc) + arc_centre[0],
                0.0,
    ...
    if DEBUG:
        ...
        _show_for_debugging(
            m, sph_centre, r_sph, P_centre, P_surround, surround_fit, M_sph
        )
